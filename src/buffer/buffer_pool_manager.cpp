...
Page *BufferPoolManager::FetchPageImpl(page_id_t page_id) {
  ...
  if (got != page_table_.end()) {
    ...
    if (page->pin_count_++ == 0) replacer_->Pin(got->second);
    return page;
  }
  // 1.2   If all the pages in the buffer pool are pinned, return nullptr.
  if (free_list_.empty() && replacer_->Size() == 0) {
    return nullptr;
  }
  // 2.   Pick a victim page
...
bool BufferPoolManager::UnpinPageImpl(page_id_t page_id, bool is_dirty) {
  ...
  if (--page->pin_count_ == 0) replacer_->Unpin(got->second);
  return true;
}
...
Page *BufferPoolManager::NewPageImpl(page_id_t *page_id) {
  ...
  // 2.   Make sure you call DiskManager::AllocatePage!
...
bool BufferPoolManager::DeletePageImpl(page_id_t page_id) {
  ...
  if (got == page_table_.end()) {
    ...
    replacer_->Pin(offset);  // Remove from replace, since the pin count is 0
